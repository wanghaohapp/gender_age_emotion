        ...
        if numbox > 0:
            ...
            if total_boxes.shape[0] > 0:
                ...
                if len(pick) > 0:
                    ...
                    points = points[pick, :]
    if (total_boxes.shape[0] > 0):
        keypoints = np.zeros((points.shape[0], 5, 2)).astype(np.float32)
        for i in range(points.shape[0]):
            for j in range(5):
                keypoints[i][j][0] = points[i][j]
                keypoints[i][j][1] = points[i][j + 5]
        return total_boxes, keypoints
    else:
